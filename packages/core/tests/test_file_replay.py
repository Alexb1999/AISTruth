...
@pytest.mark.asyncio
async def test_file_replay_fetch_for_mmsi(tmp_path: Path) -> None:
    fixture = tmp_path / "track.jsonl"
    fixture.write_text(
        '{"mmsi":257000000,"time":"2026-01-01T00:00:00Z","lat":60.0,"lon":5.0}\n'
        '{"mmsi":257000001,"time":"2026-01-01T00:00:00Z","lat":61.0,"lon":6.0}\n',
        encoding="utf-8",
    )
    source = FileReplayAisSource(fixture)
    reports = await source.fetch_reports_for_mmsi(257000000)
    assert len(reports) == 1
    assert reports[0].mmsi == 257000000
